...
- **v1.30 (19-ago-2026, más noche) · El envío a chorro: la memoria ya no revienta.** Asier, tras la v1.29: «acabo de intentar compartir otra vez el chat entero con imágenes y no me ha dicho absolutamente nada». Cuando la memoria revienta DE VERDAD, muere el proceso entero y ni el aviso de la v1.29 puede salir. La raíz era el envío: `cuerpo.toString()` montaba TODO el JSON en un solo texto gigante y `HttpURLConnection` lo guardaba entero OTRA VEZ antes de enviarlo — con un export gordo, más de cien megas de golpe. Ahora el cuerpo se escribe trozo a trozo directo a la conexión (`android.util.JsonWriter` + `setChunkedStreamingMode(0)`): el texto gigante ya no existe nunca. Y el reenvío con nombre («¿de quién es esto?») también caza `Throwable`. En el servidor (backend **2.7.401**), además, cada Compartir que ENTRA manda una notificación al móvil con lo guardado — aunque el aviso de pantalla se pierda, Asier se entera siempre.

- **v1.29 (19-ago-2026, noche) · Compartir nunca muere en silencio.** Con el justificante de Adrián (ficha 6557), Asier compartió el chat dos veces y no llegó nada — y la pantalla no dijo ni mu. Tres remiendos: **(1)** el hilo de trabajo caza `Throwable`, no solo `Exception` — si el móvil se queda sin memoria (`OutOfMemoryError`, posible con un vídeo gordo tras subir el tope de lectura en la v1.28), ahora sale el aviso con el motivo en vez de morir callado. **(2)** Antes de leer un archivo se mira su PESO y su tipo (`OpenableColumns.SIZE` + mime): un vídeo de más de 12 MB o cualquier cosa de más de 40 MB se salta SIN cargarla en memoria (y CONSTA en la cuenta de «se quedaron fuera»). **(3)** Compartir SOLO un PDF (el justificante de una reserva) funcionaba de milagro: el manifest lo aceptaba desde la v1.27 pero el código lo tiraba con «No he podido leer la conversación» porque el texto de relleno solo se ponía si había fotos — ahora los PDFs sueltos también cuentan.

- **v1.28 (19-ago-2026) · Los vídeos del chat también viajan — y Azkarin los VE.** Asier: «necesito que también pueda entender los vídeos». `CompartirActivity` reconoce los vídeos por dentro (la caja `ftyp` del MP4/3GP/MOV) o por el nombre, tanto sueltos en el export CON archivos como dentro del zip del iPhone, y los manda en `videos:[{nombre,base64}]` (topes: DOS vídeos por envío y 12 MB cada uno — más gordos, el ojo de Gemini no los puede mirar). El tope total del envío sube de 24 a 32 MB, y `leerUri` ya no corta los archivos a 8 MB (un vídeo llegaría roto): ahora corta a 40. El servidor (backend **2.7.399**) los guarda en la carpeta del cliente como `WA_<nombre original>` y **mirar_fotos_whatsapp también los ve** para sacar los muebles. El aviso final dice la verdad también de los vídeos (guardados / fuera por tamaño o por ser más de dos / servidor viejo). Manifest: filtro `SEND video/*`.

- **v1.27 (19-ago-2026) · Los PDFs del chat también viajan.** Asier: «quiero que se vea lo que se ha mandado — los presupuestos y los justificantes en PDF». `CompartirActivity` reconoce los PDFs por dentro (empiezan por %PDF) o por el nombre, tanto sueltos en el export CON archivos como dentro del zip del iPhone, y los manda en `documentos:[{nombre,base64}]` (tope: diez PDFs, ocho megas cada uno, presupuesto de bytes compartido con las fotos). El servidor (backend **2.7.398**) los guarda en la carpeta del cliente como `WA_<nombre original>` — el prefijo evita pisar los documentos oficiales. El aviso final dice la verdad también de los PDFs (guardados / no guardados con motivo / servidor viejo). Manifest: filtro `SEND application/pdf`. 🛑 Y marcha atrás pedida por Asier: la conversación en PDF ya NO se genera sola («eso ya está en el WhatsApp») — solo si la pide a Azkarin.

- **v1.26 (18-ago-2026) · Compartir con Azkarin LLEVA LAS FOTOS.** Asier: «arréglamelo por favor para que se pueda hacer por el camino que yo usé». El camino que usa es Compartir → Azkarin, y por ahí solo viajaba el texto: al exportar el chat CON archivos (Android manda el .txt y cada foto como archivos sueltos, `ACTION_SEND_MULTIPLE`), Azkarin ni salía en la lista de Compartir; y del .zip del iPhone se sacaba solo el .txt. Ahora `CompartirActivity` recoge las fotos de los dos sitios (se reconocen por sus primeros bytes, no por el nombre), las manda con el texto (`fotos:[{nombre,base64,mime}]`, topes: 30 fotos / 6 MB cada / 24 MB en total) y el servidor (backend **2.7.396**) las guarda en la carpeta del cliente en Drive, apuntando en el chat cuántas venían y cuántas entraron. El aviso final dice la verdad de las fotos — incluso si el servidor fuera viejo y no supiera guardarlas, lo dice en vez de callar. Camino de envío ÚNICO (`mandar()`): el reenvío con nombre («¿de quién es esto?») lleva las mismas fotos. Espera hasta 4 min cuando van fotos (los 15 s de siempre colgarían antes de que Drive acabe). Manifest: filtros `SEND_MULTIPLE */*` y `SEND image/*`, `largeHeap` para que un export gordo no tumbe la pantalla.
